...
function jekyll(done) {
  ...
  return cp
    ...
    .on('close', done);
  // return cp
  //   .exec('jekyll', ['build'], {stdio: 'inherit'})
  //   .on('close', done);
}
 ...
 * Git Push Task
 * 
 * Push the site to the Github repo.
 * 
 * @param {*} done 
 */
 function gitPush(done) {
  notify('Pushing _site to Git...');
 ...
 * Built + Push Task
 * 
 * Running just `gulp buildPush` will:
 * - Compile the theme, SASS and JavaScript files
 * - Optimize and copy images to its folder
 * - Build the config file
 * - Compile the Jekyll site
 */
 exports.buildPush = gulp.series(gulp.parallel(js, theme, images), config, jekyll, gitPush);
